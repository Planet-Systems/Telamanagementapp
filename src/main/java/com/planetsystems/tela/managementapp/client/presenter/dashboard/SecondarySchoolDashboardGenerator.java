...
public class SecondarySchoolDashboardGenerator {

	private static SecondarySchoolDashboardGenerator instance = new SecondarySchoolDashboardGenerator();

	private SecondarySchoolDashboardGenerator() {

	}

	public static SecondarySchoolDashboardGenerator getInstance() {
		return instance;
	}

	public void generateDashboard(DashboardPane dashboardPane, final DashboardSummaryDTO dto) {

		long secondaryLearners = dto.getSecondaryLearners();
		long secondaryLearnersBoys = dto.getSecondaryLearnersBoys();
		long secondaryLearnersGirls = dto.getSecondaryLearnersGirls();

		long secondaryTeachers = dto.getSecondaryTeachers();
		long secondaryTeachersOnPayRoll = dto.getSecondaryTeachersOnPayRoll();
		long secondaryTeachersNotOnPayRoll = dto.getSecondaryTeachersNotOnPayRoll();

		long secondaryTeachersMale = dto.getSecondaryTeachersMale();
		long secondaryTeachersFemale = dto.getSecondaryTeachersFemale();

		long secondaryTeachersWithSpecialNeedslearners = dto.getSecondaryTeachersWithSpecialNeedslearners();
		long secondaryTeachersWithoutSpecialNeedslearners = dto.getSecondaryTeachersWithoutSpecialNeedslearners();

		List<DataOutPutDTO> secondarystaffEnrolledByRegion = dto.getSecondarystaffEnrolledByRegion();

		List<DataOutPutByGenderDTO> secondaryStaffEnrolledByGenderByRegion = dto
				.getSecondaryStaffEnrolledByGenderByRegion();

		List<DataOutPutDTO> secondarylearnsEnrolledByRegion = dto.getSecondarylearnsEnrolledByRegion();

		List<DataOutPutByGenderDTO> secondarylearnersEnrolledByGenderRegion = dto
				.getSecondarylearnersEnrolledByGenderRegion();

		HLayout row1Summary = new HLayout();

		row1Summary.addMember(getCard("Teachers Enrolled", UtilityManager.getInstance().formatCash(secondaryTeachers),
				"#ffffff", "#7FBA00"));

		row1Summary.addMember(getCard("Learners Enrolled", UtilityManager.getInstance().formatCash(secondaryLearners),
				"#ffffff", "#00A4EF"));

		row1Summary.addMember(getCard("Teachers on Payroll",
				UtilityManager.getInstance().formatCash(secondaryTeachersOnPayRoll), "#ffffff", "#FFB900"));
		row1Summary.addMember(getCard("Teachers Not on Payroll",
				UtilityManager.getInstance().formatCash(secondaryTeachersNotOnPayRoll), "#ffffff", "#737373"));
		...
		chart1.addMember(tearchersByGenderChart(secondaryTeachersMale, secondaryTeachersFemale,
				dto.getYear() + "-" + dto.getTerm()));

		VLayout chart2 = new VLayout();
		chart2.addMember(learnerByGenderCountChart(secondaryLearnersBoys, secondaryLearnersGirls,
				dto.getYear() + "-" + dto.getTerm()));

		VLayout chart3 = new VLayout();
		chart3.addMember(staffBySpecialNeedsCountChart(secondaryTeachersWithSpecialNeedslearners,
				secondaryTeachersWithoutSpecialNeedslearners, dto.getYear() + "-" + dto.getTerm()));

		VLayout chart4 = new VLayout();
		chart4.addMember(teachersPerRegionChart(secondarystaffEnrolledByRegion));

		VLayout chart5 = new VLayout();
		chart5.addMember(teachersByRegionByGenderStackedChart(secondaryStaffEnrolledByGenderByRegion));

		VLayout chart6 = new VLayout();
		chart6.addMember(learnersPerRegionChart(secondarylearnsEnrolledByRegion));

		VLayout chart7 = new VLayout();
		chart7.addMember(learnersByRegionByGenderStackedChart(secondarylearnersEnrolledByGenderRegion));
