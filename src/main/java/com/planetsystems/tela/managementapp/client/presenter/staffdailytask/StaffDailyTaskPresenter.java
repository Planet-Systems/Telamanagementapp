...
import com.smartgwt.client.widgets.grid.ListGridRecord;import java_cup.internal_error;
...
final TextItem dayItem = getView().getStaffDailyTaskPane().getDayField();
  ...
  termBox.addChangedHandler(new ChangedHandler() {
	...
	public void onChanged(ChangedEvent event) {
		...
		if (termBox.getValueAsString() != null && schoolBox.getValueAsString() != null  && staffBox.getValueAsString() != null && dayItem.getValueAsString() != null) {
		...
		public void onClick(ClickEvent event) {
			...
			if (getView().getStaffDailyTaskPane().getLessonListGrid().anySelected()) {
			    ...
			    // staffDailyAttendanceDTO.setId(id);
			     ...
			     ListGridRecord[] records = getView().getStaffDailyTaskPane().getLessonListGrid().getSelectedRecords();
			     ...
			     for(int i = 0 ; i<records.length ; i++ ) {
			    	 ...
			    	 taskDTO.setSchoolClassDTO(new SchoolClassDTO(record.getAttributeAsString(LessonListGrid.CLASS_ID)));
