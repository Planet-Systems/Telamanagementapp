...
public class TimeTablePresenter extends Presenter<TimeTablePresenter.MyView, TimeTablePresenter.MyProxy> {
			...
			public void onClick(ClickEvent event) {
				...
				createTimeTablePane.getSaveButton().addClickHandler(new ClickHandler() {
					...
					public void onClick(ClickEvent event) {
						...
						ListGridRecord[] records = createTimeTablePane.getLessonListGrid().getRecords(); //new ListGridRecord[list.size()];
						...
						for (int i = 0; i < records.length; i++) {
							...
							SchoolClassDTO schoolClassDTO = new SchoolClassDTO(record.getAttribute(LessonListGrid.CLASS_ID));
							lessonDTO.setSchoolClassDTO(schoolClassDTO);
							
							SchoolStaffDTO schoolStaffDTO = new SchoolStaffDTO(record.getAttribute(LessonListGrid.STAFF_ID));
	...
	private void saveTimeTable(final TimeTableDTO timeTableDTO , final Tab lessonTab , final String defaultValue) {
		  LinkedHashMap<String, Object> map = new LinkedHashMap<>();
			...
			dispatcher.execute(new RequestAction(RequestConstant.SAVE_TIME_TABLE , map),
	...
	private void displayTimeTableLessonWindow(final CreateTimeTablePane createTimeTablePane) {
		createTimeTablePane.getAddLessonButton().addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				SC.say("SCHOOOL "+createTimeTablePane.getSchoolComboBox().getValueAsString());
...
//				   dto.setStatus(status);
	...
	private boolean checkIfNoTimeTableLessonFieldIsEmpty(TimeTableLessonWindow window) {
		...
		if(window.getSchoolClassCombo().getValueAsString() == null) flag = false;
		
		if(window.getSchoolClassCombo().getValueAsString() == null) flag = false;
		if(window.getSubjectCombo().getValueAsString() == null) flag = false;
		if(window.getStaffCombo().getValueAsString() == null) flag = false;
		if(window.getDayCombo().getValueAsString() == null) flag = false;
		if(window.getStartTime().getEnteredValue() == null) flag = false;
		if(window.getEndTime().getEnteredValue() == null) flag = false;
	...
	private void loadAcademicTermsInAcademicYearCombo(final CreateTimeTablePane createTimeTablePane, final String defaultValue) {
	...
	private void loadClassesInSchoolCombo(final TimeTableLessonWindow window , final String schoolId, final String defaultValue) {
			SC.say("Changing school");
		//  String schoolId = createTimeTablePane.getSchoolComboBox().getValueAsString();
	...
	private void loadStaffsInSchoolCombo(final TimeTableLessonWindow window , String schoolId , final String defaultValue) {
	...
	private void loadSubjectsCombo(final TimeTableLessonWindow window , final String defaultValue) {
		  ...
		  LinkedHashMap<String, Object> map = new LinkedHashMap<>();
			...
			dispatcher.execute(new RequestAction(RequestConstant.GET_SUBJECT , map),
					new AsyncCallback<RequestResult>() {
						public void onFailure(Throwable caught) {
							System.out.println(caught.getMessage());
							SC.warn("ERROR", caught.getMessage());
